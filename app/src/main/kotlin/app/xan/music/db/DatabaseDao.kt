...
import app.xan.music.db.entities.AlbumArtistMap
import app.xan.music.db.entities.AlbumEntity
import app.xan.music.db.entities.AlbumWithSongs
...
import app.xan.music.db.entities.EventWithSong
import app.xan.music.db.entities.FormatEntity
import app.xan.music.db.entities.LyricsEntity
import app.xan.music.db.entities.PlayCountEntity
...
import app.xan.music.db.entities.RecognitionHistory
import app.xan.music.db.entities.RelatedSongMap
import app.xan.music.db.entities.SearchHistory
import app.xan.music.db.entities.SetVideoIdEntity
import app.xan.music.db.entities.Song
import app.xan.music.db.entities.SongAlbumMap
import app.xan.music.db.entities.SongAnalysisEntity
import app.xan.music.db.entities.SongArtistMap
import app.xan.music.db.entities.SongEntity
import app.xan.music.db.entities.SongWithStats
import app.xan.music.extensions.reversed
import app.xan.music.extensions.toSQLiteQuery
...
internal const val MOST_PLAYED_SONGS_STATS_QUERY = """
        SELECT s.id, s.title, COALESCE(s.thumbnailUrl, '') AS thumbnailUrl, s.isVideo,
               ...
               COALESCE((SELECT SUM(event.playTime)
                FROM event
                WHERE songId = s.id
                  AND timestamp > :fromTimeStamp AND timestamp <= :toTimeStamp), 0) AS timeListened
        FROM song s
        ...
        JOIN (SELECT songId
              ...
              GROUP BY songId
              ORDER BY COALESCE(SUM(playTime), 0) DESC
...
interface DatabaseDao {
    ...
    @Transaction
    @Query(MOST_PLAYED_SONGS_STATS_QUERY)
