...
import app.xan.music.constants.ListenTogetherAutoAddSuggestionsKey
import app.xan.music.constants.ListenTogetherAutoApprovalKey
import app.xan.music.constants.ListenTogetherIsHostKey
import app.xan.music.constants.ListenTogetherRoomCodeKey
import app.xan.music.constants.ListenTogetherServerUrlKey
import app.xan.music.constants.ListenTogetherSessionTimestampKey
import app.xan.music.constants.ListenTogetherSessionTokenKey
import app.xan.music.constants.ListenTogetherUserIdKey
...
class ListenTogetherClient @Inject constructor(
    ...
    companion object {
        ...
        const val ACTION_APPROVE_JOIN = "app.xan.music.LISTEN_TOGETHER_APPROVE_JOIN"
        const val ACTION_REJECT_JOIN = "app.xan.music.LISTEN_TOGETHER_REJECT_JOIN"
        const val ACTION_APPROVE_SUGGESTION = "app.xan.music.LISTEN_TOGETHER_APPROVE_SUGGESTION"
        const val ACTION_REJECT_SUGGESTION = "app.xan.music.LISTEN_TOGETHER_REJECT_SUGGESTION"
    ...
    private fun loadBlockedUsernames() {
        try {
            val blockedJson = context.dataStore.get(app.xan.music.constants.ListenTogetherBlockedUsersKey, "")
    ...
    private suspend fun saveBlockedUsernames() {
        try {
            ...
            context.dataStore.edit { preferences ->
                preferences[app.xan.music.constants.ListenTogetherBlockedUsersKey] = blockedJson
            }
        } catch (e: Exception) {
    ...
    fun connect(roomCode: String? = null) {
        ...
        if (getServerUrl().isBlank()) {
            ...
            scope.launch {
                _events.emit(ListenTogetherEvent.ConnectionError("Set a sync server URL in Listen Together settings"))
            }
            return
        }
