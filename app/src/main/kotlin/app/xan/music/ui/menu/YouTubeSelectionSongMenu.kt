    ...
    AddToPlaylistDialogOnline(
        ...
        songs = remember { 
            songSelection.map { song ->
                ...
                val metadata = song.toMediaMetadata()
                app.xan.music.db.entities.Song(
                    song = app.xan.music.db.entities.SongEntity(
                        id = metadata.id,
                    ...
                    artists = metadata.artists.map { artist ->
                        app.xan.music.db.entities.ArtistEntity(
                            id = artist.id ?: "",
                            name = artist.name
                        )
                    },
                    album = metadata.album?.let { album ->
                        app.xan.music.db.entities.AlbumEntity(
                            id = album.id,
                            title = album.title,
                        ...
                        Download.STATE_QUEUED, Download.STATE_DOWNLOADING -> {
                            ...
                            Material3MenuItemData(
                                ...
                                onClick = {
                                    downloadSongs(
                                        context,
                                        songSelection.map { it.toMediaMetadata().toDownloadTarget() },
                                        downloads,
                                    )
                                    clearAction()
                                    onDismiss()
                            ...
                            database.transaction {
                                songSelection.forEach { song ->
                                    ...
                                    if ((!allLiked && !metadata.liked) || allLiked) {
                                        ...
                                        val songEntity = app.xan.music.db.entities.SongEntity(
                                            id = metadata.id,
                                            title = metadata.title,
                                            duration = metadata.duration,
                                            thumbnailUrl = metadata.thumbnailUrl,
