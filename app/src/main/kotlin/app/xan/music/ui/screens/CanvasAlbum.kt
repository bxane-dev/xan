 ...
 * player background (see Player.kt): Echo/XAN are song-keyed so they're
...
fun rememberAlbumCanvas(
    ...
    LaunchedEffect(albumTitle, artistName, firstSongTitle) {
        ...
        val fetched = withContext(Dispatchers.IO) {
            ...
            val xan = XanCanvasProvider.getBySongArtist(songQuery, artistName)
                ?.takeIf { !it.preferredAnimationUrl.isNullOrBlank() }
                ?.takeIf { artistOnlyMatches(it.artist, artistName) }
            ...
            listOfNotNull(echo, apple, xan, tidal)
        }
