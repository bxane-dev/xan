...
open class MusicWidgetReceiver : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ACTION_PLAY_PAUSE, ACTION_LIKE, ACTION_NEXT, ACTION_PREVIOUS -> {
                ...
                // Use startForegroundService so the system grants FGS privilege
                // on Android 12+ (API 31+) where background FGS starts are banned.
    ...
    companion object {
        const val ACTION_PLAY_PAUSE = "app.xan.music.widget.PLAY_PAUSE"
        const val ACTION_LIKE = "app.xan.music.widget.LIKE"
        const val ACTION_NEXT = "app.xan.music.widget.NEXT"
        const val ACTION_PREVIOUS = "app.xan.music.widget.PREVIOUS"
        const val ACTION_UPDATE_WIDGET = "app.xan.music.widget.UPDATE_WIDGET"
    }
}
