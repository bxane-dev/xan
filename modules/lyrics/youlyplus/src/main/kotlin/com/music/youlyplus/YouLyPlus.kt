...
object YouLyPlus {
    ...
    suspend fun getLyrics(
        ...
        try {
            // Prefer true word/syllable sync over whichever mirror responds first.
            // Return immediately for word-sync; otherwise retain the best fallback
            // while the remaining mirrors finish.
            ...
            while (remaining.isNotEmpty()) {
                ...
                if (quality >= 3) {
                    lastWorkingServer.set(server)
                    return@runCatching lrc
                }
            }

            bestLyrics?.also { lastWorkingServer.set(bestServer) }
    ...
    private fun lyricQuality(lrc: String): Int {
        if (lrc.isBlank()) return 0
        val wordTimed =
            Regex("<\\d{1,2}:\\d{2}\\.\\d{2,3}>").containsMatchIn(lrc) ||
                Regex("(?m)^<[^>]+:\\d+(?:\\.\\d+)?:\\d+(?:\\.\\d+)?(?:\\|[^>]+)*>$").containsMatchIn(lrc)
        if (wordTimed) return 3
        if (Regex("\\[\\d{1,2}:\\d{2}\\.\\d{2,3}]").containsMatchIn(lrc)) return 2
        return 1
    }
