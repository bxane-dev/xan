...
object Spotify {
    ...
    suspend fun myPlaylists(
        ...
        val playlists = rawItems?.mapNotNull { itemElem ->
            val row = runCatching { itemElem.jsonObject }.getOrNull() ?: return@mapNotNull null
            val wrapper = row.obj("item") ?: return@mapNotNull null
            if (wrapper.str("__typename") != "PlaylistResponseWrapper") return@mapNotNull null
            runCatching { parsePlaylistWrapper(wrapper) }.getOrNull()
        } ?: emptyList()
    ...
    suspend fun playlistTracks(
        ...
        val normalizedPlaylistId = SpotifyPlaylistRefParser.parse(playlistId)
            ?: throw SpotifyException(400, "Invalid Spotify playlist URL, URI, or ID")
        val vars = buildJsonObject {
            put("uri", "spotify:playlist:$normalizedPlaylistId")
            put("offset", offset)
            put("limit", limit)
        ...
        val playlistData = response.obj("data")?.obj("playlistV2")
            ?: throw SpotifyException(500, "Invalid fetchPlaylist response")
        val content = playlistData.obj("content")
            ?: playlistData.obj("tracksV2")
            ?: playlistData.obj("tracks")
            ?: throw SpotifyException(500, "No track content in fetchPlaylist response")

        val rawItems = content.arr("items")
        val tracks = rawItems?.mapNotNull { elem ->
            val row = runCatching { elem.jsonObject }.getOrNull() ?: return@mapNotNull null
            val itemWrapper = row.obj("itemV2")
                ?: row.obj("item")
                ?: row.obj("track")
                ?: row
            val itemData = itemWrapper.obj("data")
                ?: itemWrapper.obj("track")?.obj("data")
                ?: itemWrapper.obj("track")
                ?: return@mapNotNull null

            // Episodes/unavailable placeholders share playlist pages on some accounts.
            // Skip only that row; never truncate the remaining page.
            if (itemData.str("__typename")?.contains("Episode", ignoreCase = true) == true) {
                return@mapNotNull null
            }

            val wrapperUri = itemWrapper.str("_uri")
                ?: itemWrapper.str("uri")
                ?: itemData.str("_uri")
                ?: itemData.str("uri")
            val track = runCatching { parseGqlTrack(itemData, uriOverride = wrapperUri) }
                .getOrNull() ?: return@mapNotNull null
            if (track.id.isBlank() || track.name.isBlank()) return@mapNotNull null

            SpotifyPlaylistTrack(
                track = track,
                uid = row.str("uid") ?: itemWrapper.str("uid"),
            )
        } ?: emptyList()
    ...
    suspend fun likedSongs(
        ...
        val rawItems = tracksData.arr("items")
        val savedTracks = rawItems?.mapNotNull { elem ->
            val row = runCatching { elem.jsonObject }.getOrNull() ?: return@mapNotNull null
            val trackWrapper = row.obj("track") ?: return@mapNotNull null
            val trackData = trackWrapper.obj("data") ?: return@mapNotNull null
            val wrapperUri = trackWrapper.str("_uri") ?: trackWrapper.str("uri")
            val track = runCatching { parseGqlTrack(trackData, uriOverride = wrapperUri) }
                .getOrNull() ?: return@mapNotNull null
            if (track.id.isBlank() || track.name.isBlank()) return@mapNotNull null
