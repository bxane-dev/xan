...
data class SpotifyPaging<T>(
    ...
    val limit: Int = 20,
    val offset: Int = 0,
    val rawItemCount: Int = items.size,
)

fun <T> SpotifyPaging<T>.nextOffsetOrNull(currentOffset: Int, requestedLimit: Int): Int? {
    if (rawItemCount <= 0) return null
    val next = currentOffset + rawItemCount
    if (total > 0 && next >= total) return null
    if (total <= 0 && rawItemCount < requestedLimit) return null
    return next
}
