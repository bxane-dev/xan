package app.xan.music.xancanvas
...
@Serializable
data class XanCanvasManifest(
    val items: List<XanCanvasItem> = emptyList()
)

@Serializable
data class XanCanvasItem(
    val song: String,
    val artist: String,
    val url: String
)

object XanCanvasProvider {
    private const val BASE_URL = "https://raw.githubusercontent.com/bxane-dev/xan/main/canvas.json"
    ...
    private suspend fun fetchManifest(): XanCanvasManifest? = fetchMutex.withLock {
        ...
        try {
            val manifest: XanCanvasManifest = client.get(BASE_URL).body()
